...
function getInkRGB() {
  return document.documentElement.hasAttribute("data-theme")
    ? "30,28,24"     // light mode: dark ink on cream
    : "237,233,223"; // dark mode:  warm ivory on dark bg
}
...
export function useKaleidoscope(canvasId: string) {
  const stateRef = useRef<KState>({
    scroll: 0, scrollNorm: 0, scrollVel: 0,
    lastScroll: 0, time: 0, burst: 0,
  });
  ...
  useEffect(() => {
    ...
    function drawKaleidoscope(cx: number, cy: number, params: KParams) {
      const {
        baseRadius, segments, rings, outerRot, innerRot,
        centerRot, opacity, burstScale, polygonSides,
      } = params;
      ...
      ctx!.save();
      ctx!.translate(cx, cy); ctx!.rotate(outerRot); ctx!.translate(-cx, -cy);

      for (let i = 0; i < segments; i++) {
        const a  = (i / segments) * TWO_PI;
        const lw = (i % 2 === 0 ? 0.6 : 0.3) * LW;
        drawSpoke(cx, cy, R, a, `rgba(${INK_RGB},${alpha})`, lw);
      }
      for (let j = 1; j <= rings; j++) {
        const r   = (R * j) / rings;
        const lw  = (j === rings ? 0.5 : 0.35) * LW;
      ...
      for (let j = 0; j < rings - 1; j++) {
        ...
        drawPolygon(cx, cy, rm, polygonSides, off, `rgba(${INK_RGB},${alpha * 0.6})`, 0.3 * LW, j % 2 ? [5, 9] : null);
      }
      ctx!.restore();
      ...
      ctx!.save();
      ctx!.translate(cx, cy); ctx!.rotate(innerRot); ctx!.translate(-cx, -cy);

      for (let i = 0; i < innerSegs; i++) {
        const a = (i / innerSegs) * TWO_PI;
        drawSpoke(cx, cy, innerR, a, `rgba(${INK_RGB},${alpha * 0.7})`, 0.35 * LW);
      }
      for (let j = 1; j <= innerRings; j++) {
        const r   = (innerR * j) / innerRings;
        const dsh = j === innerRings ? [3, 6] : null;
        drawCircle(cx, cy, r, `rgba(${INK_RGB},${alpha * 0.6})`, 0.3 * LW, dsh);
      }
      drawPolygon(cx, cy, innerR * 0.85, innerSegs, innerRot * 0.5, `rgba(${INK_RGB},${alpha * 0.45})`, 0.25 * LW, null);
      ctx!.restore();

      // ── Layer 3: center star cluster ──
      const starR  = R * 0.32;
      const starR2 = R * 0.2;
      const starR3 = R * 0.13;

      ctx!.save();
      ctx!.translate(cx, cy); ctx!.rotate(centerRot); ctx!.translate(-cx, -cy);

      drawStar(cx, cy, starR,  starR  * 0.38, 8, 0,           `rgba(${INK_RGB},${alpha * 1.1})`,  0.7  * LW);
      drawStar(cx, cy, starR,  starR  * 0.38, 8, Math.PI / 8, `rgba(${INK_RGB},${alpha * 0.45})`, 0.35 * LW);
      drawStar(cx, cy, starR2, starR2 * 0.4,  8, 0,           `rgba(${INK_RGB},${alpha * 0.9})`,  0.5  * LW);
      drawStar(cx, cy, starR3, starR3 * 0.4,  8, Math.PI / 8, `rgba(${INK_RGB},${alpha * 0.7})`,  0.4  * LW);
      ...
      // Center dot
      drawCircle(cx, cy, starR * 0.05, `rgba(${INK_RGB},${alpha})`,        0.6  * LW, null);
      drawCircle(cx, cy, starR * 0.12, `rgba(${INK_RGB},${alpha * 0.5})`,  0.35 * LW, null);
      ctx!.restore();

      // ── Layer 4: corner accent marks ──
      const corners = [
      ...
      corners.forEach(({ x, y }) => {
        ...
        ctx!.beginPath();
        ctx!.moveTo(x - accentSize, y); ctx!.lineTo(x + accentSize, y);
        ctx!.moveTo(x, y - accentSize); ctx!.lineTo(x, y + accentSize);
        ctx!.strokeStyle = `rgba(${INK_RGB},${aAlpha})`;
        ctx!.lineWidth   = 0.45 * LW;
        ctx!.stroke();
        drawCircle(x, y, accentSize * 0.45, `rgba(${INK_RGB},${aAlpha * 0.8})`, 0.35 * LW, null);
        drawPolygon(x, y, accentSize * 0.7, 4, Math.PI / 4, `rgba(${INK_RGB},${aAlpha * 0.5})`, 0.3 * LW, null);
      });
    }
    ...
    function computeParams(t: number, sNorm: number, vel: number): KParams {
      ...
      return {
        baseRadius: baseR, segments, rings, polygonSides,
        outerRot, innerRot, centerRot, opacity, burstScale, scrollNorm: sNorm,
      };
    }

    // ── Satellite kaleidoscopes ────────────────────────────────────────
